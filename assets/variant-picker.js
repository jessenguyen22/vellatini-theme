if (!customElements.get("variant-picker")) {
  class VariantPicker extends HTMLElement {
    ...
    setupMediaGallery() {
      ...
      // Nếu trước đó chưa tìm thấy mapping vì DOM chưa render, thử build lại
      if (!this.variantGroupImages || !this.variantGroupImages.enable) {
        const built = this.buildVariantGroupFromAlt();
        if (built && built.enable) {
          this.variantGroupImages = built;
          // Đánh dấu cần update media sau khi slider / mediaItems đã sẵn sàng
          this._needInitialMediaUpdate = true;
        }
      }
      ...
      if (this._needInitialMediaUpdate && this.currentVariant) {
        this.getDataImageVariant(this.currentVariant.id);
        this.updateMedia();
        this._needInitialMediaUpdate = false;
      }
    }
    ...
    getDataImageVariant(variantId) {
      ...
      let keys = [String(variantId).toUpperCase()];
      if (variant && variant.options) {
        keys = keys.concat(variant.options.map((o) => String(o).trim().toUpperCase()));
      }

      const currentVariantMedia = this.variantGroupImages.mapping.find((map) => {
        return keys.includes(String(map.id).trim().toUpperCase());
      });

      if (currentVariantMedia) {
        this.currentVariantMedia = currentVariantMedia.media.map((id) => String(id));
      }
    }
    ...
    getVariantGroupImageData() {
      ...
      // Nếu vẫn chưa có, build từ ALT text
      if (!this.variantGroupImages || !this.variantGroupImages.enable) {
        this.variantGroupImages = this.buildVariantGroupFromAlt();
      }
      return this.variantGroupImages || {};
    }

    // Xây mapping từ ALT-text: [VG:Black] hoặc [VG:5052157]
    buildVariantGroupFromAlt() {
      const imagesMap = {};

      // Ưu tiên quét DOM để chắc chắn có ALT & ID
      const domMedias = this.productWrapper.querySelectorAll('.m-product-media--item');

      domMedias.forEach((item) => {
        const altRaw = (item.dataset.mediaAlt || "").toUpperCase();
        const mediaId = String(item.dataset.mediaId || "");
        const matches = altRaw.match(/\[VG:([^\]]+)\]/g);
        if (!matches) return;

        matches.forEach((tag) => {
          const inner = tag.replace(/\[VG:|\]/g, "");
          inner.split(',').forEach((token) => {
            const raw = token.trim();
            if (!raw) return;
            const key = isNaN(raw) ? raw.toUpperCase() : Number(raw);
            if (!imagesMap[key]) imagesMap[key] = [];
            imagesMap[key].push(mediaId);
          });
        });
      });

      // Fallback nếu DOM chưa sẵn: quét productData
      if (Object.keys(imagesMap).length === 0) {
        const medias = (this.productData && this.productData.media) || [];
        medias.forEach((m) => {
          const altText = m.alt || (m.preview_image && m.preview_image.alt) || "";
          const altUpper = altText.toUpperCase();
          const matches = altUpper.match(/\[VG:([^\]]+)\]/g);
          if (!matches) return;
          ...
          matches.forEach((tag) => {
            ...
            inner.split(',').forEach((token) => {
              ...
              const key = isNaN(raw) ? raw.toUpperCase() : Number(raw);
              if (!imagesMap[key]) imagesMap[key] = [];
              imagesMap[key].push(String(m.id));
            });
          });
        });
      }

      const mapping = Object.keys(imagesMap).map((k) => ({
        id: isNaN(k) ? k : Number(k),
        media: imagesMap[k],
      }));

      return { enable: mapping.length > 0, mapping };
    }
